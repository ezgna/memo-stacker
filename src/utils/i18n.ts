...
const i18n = new I18n({
  en: {
    ...
    remove_ads_answer: 'You can permanently remove all ads with a one-time purchase from the Settings page.\nAfter purchasing, ads will be hidden on all devices using the same Apple or Google account (you may need to restore your purchase).\nHowever, if you switch between iOS and Android, the purchase cannot be restored, and you will need to purchase it again.',
    freePlan: "Free Plan",
  ...
  ja: {
    ...
    search_notes_answer: 'ホーム画面右上の検索バーから、特定キーワードを含むメモを検索できます。\n日本語ではひらがなとカタカナが区別されます（例:「あ」と検索すると「あ」を含むメモが表示され、「ア」は表示されません）。\n英語では大文字と小文字の区別はありません（例:「A」と検索すると「a」と「A」を含むメモが表示されます）。',
    edit_delete_notes_question: 'メモの編集・削除',
    edit_delete_notes_answer: "各メモの右上の三点リーダをタップすると、編集や削除ができます。削除したメモはゴミ箱に7日間保存され、ゴミ箱（タブ内）からいつでも復元できます。7日を過ぎると端末から完全に削除され、復元はできません。\nゴミ箱内のメモを復元するには、同じように三点リーダをタップしてください。",
