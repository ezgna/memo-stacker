...
const i18n = new I18n({
  en: {
    ...
    edit_delete_notes_answer: "On the home screen or in the tab view, you can edit or delete a memo by tapping the three-dot menu at the top right of the memo.\nFree plan: There is no trash feature, but a confirmation alert will appear before deletion. Once deleted, the memo disappears from view but is retained internally (you can recover it by upgrading to the Pro plan within 7 days).\nPro plan: Deleted memos are stored in the trash for 7 days and can be recovered at any time. After 7 days, the memo is permanently deleted from both the device and the cloud.\nTo recover a memo from the trash, tap the three-dot menu in the same way.",
    security_info_question: "Is it secure?",
    security_info_answer: "Free plan: Memos are saved only on the device, and they are safe as long as the device itself is not accessed.\nPro plan: Memos are saved both on the device and in the cloud. They are encrypted during upload to the cloud for safety.\nUnless the device is directly accessed by a third party, the contents of the memos will not be leaked.",
    device_transfer_question: "How to transfer data when changing devices",
    device_transfer_answer: "Supported only on iOS.\nFree plan: Export your data from the settings page (signup required). Google Drive is used, so a Google account is necessary. After exporting, ensure the file is saved in Google Drive, then import it on your new device.\nPro plan: Simply log into MemoLog with the same account, and your memos will automatically sync.",
    ...
    platform_availability_question: "Is there an Android or PC version?",
    platform_availability_answer: "Currently in development.\nThe Android version is expected to be released within a few months, and the PC (browser) version is planned for release in 2025. \nMac users can install the app from the App Store, just like on iPhone.",
    ...
    language_settings_answer: "The app supports both Japanese and English.\nIt follows the language settings of your device. To change the language, adjust your device's settings.\nThe app's language will automatically switch accordingly.",
    freePlan: "Free Plan",
  ...
  ja: {
    ...
    memoLog_overview_answer: 'メモログは、閲覧性に特化したメモアプリです。\nメモは時系列順に保存されるため、日記のように使うこともできます。',
    ...
    platform_availability_answer: '現在開発中です。\nAndroid版は数ヶ月以内に公開予定で、PC（ブラウザ）版は2025年内の公開を予定しています。\nMacの方はiPhoneと同様にApp Storeからインストールできます。',
