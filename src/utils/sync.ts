...
export const syncAllDataFromSupabase = async (db: SQLite.SQLiteDatabase | null) => {
  ...
  const values = newEntries.reduce((acc, entry) => {
    return acc.concat([entry.created_at, entry.date, entry.text, entry.user_id]);
  }, []);

  await db.withTransactionAsync(async () => {
    await db.runAsync(`INSERT INTO entries (created_at, date, text, user_id) VALUES ${placeholders}`, values);
  });
};
